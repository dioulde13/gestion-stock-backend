...
// const SECRET_KEY = process.env.JWT_SECRET || "votre_secret_key";
...
const creerVendeur = async (req, res) => {
  ...
  try {
    ...
    let roleVendeur = await Role.findOne({ where: { nom: 'VENDEUR' }, transaction: t });
    if (!roleVendeur)
    ...
    const typesCaisses = [
      "PRINCIPALE","VALEUR_STOCK_PUR","CAISSE","BENEFICE",
      "VALEUR_STOCK","CREDIT_VENTE","BENEFICE_CREDIT",
      "CREDIT_ACHAT","ACHAT_ESPACE","CREDIT_ESPECE","CREDIT_ESPECE_ENTRE"
    ];
...
const connexionUtilisateur = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: utilisateur.id, email: utilisateur.email, role: utilisateur.Role.nom },
