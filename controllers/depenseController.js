...
const ajouterDepense = async (req, res) => {
  ...
  if (!montant || !description) return res.status(400).json({ message: "Tous les champs sont obligatoires." });
  ...
  try {
    ...
    console.error("Erreur lors de l'ajout de la dépense :", error.message || error);
...
const consulterDepense = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const depense = await Depense.findByPk(id, { include: [{ model: Utilisateur, attributes: ["id", "nom", "boutiqueId"] }] });
    if (!depense) return res.status(404).json({ message: "Dépense non trouvée." });

    if (utilisateur.Role.nom === "VENDEUR" && depense.utilisateurId !== utilisateur.id)
    ...
    if (utilisateur.Role.nom === "ADMIN") {
      ...
      if (boutique?.utilisateurId !== utilisateur.id && depense.utilisateurId !== utilisateur.id)
...
const modifierDepense = async (req, res) => {
  ...
  try {
    ...
    const depense = await Depense.findByPk(id, { include: [{ model: Utilisateur, attributes: ["id", "boutiqueId"] }] });
    if (!depense) return res.status(404).json({ message: "Dépense non trouvée." });
    ...
    await depense.update({ montant: montant || depense.montant, description: description || depense.description });
...
const supprimerDepense = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const depense = await Depense.findByPk(id, { include: [{ model: Utilisateur, attributes: ["id", "boutiqueId"] }], transaction: t });
    if (!depense) { await t.rollback(); return res.status(404).json({ message: "Dépense non trouvée." }); }
    ...
    const boutique = await Boutique.findByPk(utilisateur.boutiqueId, { transaction: t });
    let caisseAdminBoutique = null;
    if (boutique?.utilisateurId) caisseAdminBoutique = await getCaisseByType("CAISSE", boutique.utilisateurId, t);

    caisseUtilisateur.solde_actuel += depense.montant;
    await caisseUtilisateur.save({ transaction: t });
    if (caisseAdminBoutique) { caisseAdminBoutique.solde_actuel += depense.montant; await caisseAdminBoutique.save({ transaction: t }); }
...
const annulerDepense = async (req, res) => {
  ...
  try {
    ...
    const depense = await Depense.findByPk(id, { include: [{ model: Utilisateur, attributes: ["id", "boutiqueId"] }], transaction: t });
    if (!depense) { await t.rollback(); return res.status(404).json({ message: "Dépense non trouvée." }); }
    if (depense.status === "ANNULER") { await t.rollback(); return res.status(400).json({ message: "Cette dépense est déjà annulée." }); }
    ...
    caisseUtilisateur.solde_actuel += montant;
    await caisseUtilisateur.save({ transaction: t });
    if (caisseAdminBoutique) { caisseAdminBoutique.solde_actuel += montant; await caisseAdminBoutique.save({ transaction: t }); }
