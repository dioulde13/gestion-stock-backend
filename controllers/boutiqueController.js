    ...
    try {
        ...
        const boutiques = await Boutique.findAll({
            ...
            include: [
                {
                    model: Utilisateur,
                    as: 'Admin', // admin de la boutique
                    attributes: ['id', 'nom', 'email'],
                    include: [{ model: Role, attributes: ['id', 'nom'] }]
                },
                {
                    model: Utilisateur,
                    as: 'Vendeurs', // tous les vendeurs rattachés à cette boutique
                    attributes: ['id', 'nom', 'email'],
                    include: [{ model: Role, attributes: ['id', 'nom'] }]
                }
            ]
...
const creerBoutiqueAvecAdmin = async (req, res) => {
  ...
  try {
    ...
    let admin = await Utilisateur.findOne({ where: { email: emailAdmin }, transaction: t });
    ...
    if (!admin) {
      ...
      for (const type of typesCaisses) {
        await Caisse.create({ utilisateurId: admin.id, type, solde_actuel: 0 }, { transaction: t });
      }
    }
...
const modifierBoutique = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Boutique mise à jour avec succès.', boutique });
    } catch (error) {
